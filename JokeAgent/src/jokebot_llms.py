...
class AgenticJokeState(JokeState):
    ...
    approved: bool = False
    retry_count: int = 0
    approved_jokes_embeddings: list = []  # New field to store embeddings
...
# def get_joke_embedding(joke: str) -> list:
    # return embeddings.embed_documents([joke])[0]

# def is_similar_to_approved(joke: str, threshold: float = 0.8) -> bool:
    # joke_embedding = get_joke_embedding(joke)
    # if vectorstore:
        # similarities = cosine_similarity([joke_embedding], vectorstore)
        # max_similarity = np.max(similarities)
        # return max_similarity > threshold
    # return False

# def approve_joke(joke: str):
   # if not is_similar_to_approved(joke):
      #  vectorstore.add([joke])
      #  print("Joke approved!")
    # else:
       # print("Joke is too similar to a previously approved one.")
...
def build_joke_graph(
    ...
    critic_model: str = "llama3-8b-8192",
